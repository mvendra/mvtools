...
class CustomTask(launch_jobs.BaseTask):
    ...
    def run_task(self, feedback_object, execution_name=None):
        ...
        # actual execution
        v, r = python_wrapper.exec(script, cwd, args)
        if not v:
            return False, r
        proc_result = r[0]
        proc_stdout = r[1]
        ...
        # dump outputs
        output_backup_helper.dump_output(feedback_object, save_output, proc_stdout, ("Python's stdout has been saved to: [%s]" % save_output))
        output_backup_helper.dump_output(feedback_object, save_error_output, proc_stderr, ("Python's stderr has been saved to: [%s]" % save_error_output))

        # autobackup outputs
        output_list = [("python_plugin_stdout", proc_stdout, "Python's stdout"), ("python_plugin_stderr", proc_stderr, "Python's stderr")]
        ...
        if len(proc_stderr) > 0:
            if not suppress_stderr_warnings:
                ...
                warnings = log_helper.add_to_warnings(warnings, "python's stderr has been suppressed")
