...
(project)/build/
(project)/build/linux/
(project)/build/linux/pybuild_cpp/
(project)/build/linux/pybuild_c/pybuild_cpp.py
...
class Builder():
    ...
    def standard_cpp_hook(self):
        ...
        if (self.plat == get_platform.PLAT_LINUX or self.plat == get_platform.PLAT_MACOSX) and self.compiler == "g++":
        ...
        # common linker flags
        self.linker_flags_common += standard_cpp.get_cpp_linker_flags_linux_common_gcc()
        ...
        # common linker libs
        self.linker_libs_common += standard_cpp.get_cpp_linker_libs_linux_common_gcc()

        # debug linker libs
        if self.mode == "debug" and self.compiler == "g++":
            self.linker_libs_debug += standard_cpp.get_cpp_linker_libs_linux_debug_gcc()

        # release linker libs
        if self.mode == "release" and self.compiler == "g++":
            self.linker_libs_release += standard_cpp.get_cpp_linker_libs_linux_release_gcc()
    ...
    def do_compile(self):
        ...
        for s in self.src:
            ...
            cmd += ["-c", self.src_base + s, "-o", self.tmp_full + "/" + unroll_path_dirname(s) + path_utils.replace_extension(path_utils.basename_filtered(s), ".cpp", ".o")]
            self.call_cmd(cmd)
