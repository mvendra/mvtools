...
def mkfile_contents(project_name):
    r = ".PHONY : all clean compile link\n\n"
    r += "APPNAME=%s\n\n" % project_name
    r += "BASE_SRC=../../src\n"
    r += "BASE_OBJ=../../build\n"
    r += "RUN=../../run\n\n"
    ...
    r += "else\n"
    r += "CPPFLAGSS = $(INTFLAGS)\n"
    r += "endif\n\n"
    r += "INCLUDES=-I$(BASE_SRC)\n"
    r += "LDFLAGS=\n\n"
    r += "BASE_OBJ_FULL=$(BASE_OBJ)/$(PLAT)_x$(ARCH)_$(MODE)\n"
    r += "RUN_FULL=$(RUN)/$(PLAT)_x$(ARCH)_$(MODE)\n"
    r += "ALL_OBJS=$(foreach src,$(SRC),$(BASE_OBJ_FULL)/$(src:.cpp=.o))\n"
    r += "FULL_APP_NAME=$(RUN_FULL)/$(APPNAME)\n\n"
    r += "all: compile link\n\n"
    r += "compile:\n"
    r += "\t$(foreach src,$(SRC),$(COMPILER) $(INCLUDES) $(CPPFLAGS) -c $(BASE_SRC)/$(src) -o $(BASE_OBJ_FULL)/$(src:.cpp=.o);)\n\n"
    r += "link:\n"
