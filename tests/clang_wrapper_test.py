...
class ClangWrapperTest(unittest.TestCase):
    ...
    def delegate_setUp(self):
        ...
        v, r = mvtools_test_fixture.makeAndGetTestFolder("clang_wrapper_test")
        if not v:
            return v, r
    ...
    def testExec1(self):
        ...
        v, r = clang_wrapper.exec(None, self.main_c_full, self.test_dir)
        self.assertFalse(v)
    ...
    def testExec3(self):
        ...
        v, r = clang_wrapper.exec([], [self.main_c_full], self.test_dir)
        self.assertFalse(v)
    ...
    def testExec4(self):
        ...
        with mock.patch("generic_run.run_cmd_simple", return_value=(False, "dummy_error_msg")) as dummy:
            v, r = clang_wrapper.exec(None, [self.main_c_full])
            self.assertFalse(v)
            self.assertEqual(r, "Failed running clang command: [dummy_error_msg]")
            dummy.assert_called_with(["clang", self.main_c_full], use_cwd=None)
    ...
    def testExec5(self):
        ...
        with mock.patch("generic_run.run_cmd_simple", return_value=(True, None)) as dummy:
            v, r = clang_wrapper.exec("custom_install_path", [self.main_c_full])
            self.assertTrue(v)
