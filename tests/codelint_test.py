...
class CodeLintTest(unittest.TestCase):
    ...
    def testCodelint1(self):
        ...
        v, r = codelint.codelint(test_plugins, test_plugins_params, test_filters, True, False, test_files)
        self.assertFalse(v)
    ...
    def testCodelint3(self):
        ...
        v, r = codelint.codelint(test_plugins, test_plugins_params, test_filters, True, False, test_files)
        self.assertFalse(v)
        self.assertEqual(r, ("plugins_params[lint-test-helper-cycle-pattern-match] is not a list", []))
    ...
    def testCodelint4(self):
        ...
        test_plugins_params["lint-test-helper-cycle-pattern-match"] = [1]

        expected_report = []

        v, r = codelint.codelint(test_plugins, test_plugins_params, test_filters, True, False, test_files)
        self.assertFalse(v)
        self.assertEqual(r, ("plugins_params[lint-test-helper-cycle-pattern-match]'s entry [1] is not a string", []))
    ...
    def testCodelint6(self):
        ...
        test_filters["min-line"] = "non-list"

        expected_report = []

        v, r = codelint.codelint(test_plugins, test_plugins_params, test_filters, True, False, test_files)
        self.assertFalse(v)
        self.assertEqual(r, ("filters[min-line] is not a list", []))
    ...
    def testCodelint7(self):
        ...
        test_filters["min-line"] = [7]

        expected_report = []

        v, r = codelint.codelint(test_plugins, test_plugins_params, test_filters, True, False, test_files)
        self.assertFalse(v)
        self.assertEqual(r, ("filters[min-line]'s entry [7] is not a string", []))
    ...
    def testCodelint16(self):
        ...
        test_plugins = ["nonexistent-plugin"]
        test_plugins_params = {}
        test_filters = {}
        test_files = [test_file1]
    ...
    def testCodelint17(self):
        ...
        test_filters["min-line"] = ["3"]
        test_filters["max-line"] = ["2"]

        expected_report = []

        v, r = codelint.codelint(test_plugins, test_plugins_params, test_filters, True, False, test_files)
        self.assertFalse(v)
        self.assertEqual(r, ("min-line [3] is larger than max-line [2]", []))
    ...
    def testCodelint24(self):
        ...
        test_file1 = path_utils.concat_path(self.test_dir, "file1.txt")
        create_and_write_file.create_file_contents_hex(test_file1, "ffffff")
        ...
        v, r = codelint.codelint(test_plugins, test_plugins_params, test_filters, False, False, test_files)
        self.assertFalse(v)
        self.assertEqual(r, ("File [%s] is not UTF8 decodable" % test_file1, expected_report))

    def testCodelint25(self):

        test_file0 = path_utils.concat_path(self.test_dir, "file0.bin")
        test_file1 = path_utils.concat_path(self.test_dir, "file1.txt")

        create_and_write_file.create_file_contents_hex(test_file0, "ffffff")
        create_and_write_file.create_file_contents(test_file1, "first-line\nsecond-line\nthird-line\nfourth-line\nfifth-line")
        ...
        expected_report = []
        expected_report.append((False, "Skipped processing [%s] for being non-utf8" % test_file0))
        expected_report.append((False, "Processing [%s] - begin" % test_file1))
        expected_report.append((False, "Plugin: [lint_test_helper.py] - begin"))
        expected_report.append((True, "detected pattern [third-line] at line [3]"))
        expected_report.append((False, "Plugin: [lint_test_helper.py] - end (detected 1 finding, applied 1 patch)"))
        ...
        v, r = codelint.codelint(test_plugins, test_plugins_params, test_filters, True, True, test_files)
        self.assertTrue(v)
        self.assertEqual(r, expected_report)
    ...
    def testCodelint38(self):
        ...
        test_plugins_params["lint-test-helper-cycle-pattern-match"] = ["everyline"]
        test_plugins_params["lint-test-helper-cycle-pattern-replace"] = ["modified-everyline"]

        test_filters["max-findings"] = ["3"]

        expected_report = []
        expected_report.append((False, "Processing [%s] - begin" % test_file1))
        expected_report.append((False, "Plugin: [lint_test_helper.py] - begin"))
        expected_report.append((True, "detected pattern [everyline] at line [1]"))
        expected_report.append((True, "detected pattern [everyline] at line [2]"))
        expected_report.append((True, "detected pattern [everyline] at line [3]"))
        expected_report.append((False, "Plugin: [lint_test_helper.py] - end (detected 3 findings, applied 3 patches)"))
        expected_report.append((False, "Processing [%s] - end" % test_file1))
        ...
        self.assertEqual(getcontents.getcontents(test_file1), "modified-everyline\nmodified-everyline\nmodified-everyline\neveryline\neveryline")
