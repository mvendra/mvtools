...
class BazelPluginTest(unittest.TestCase):
    ...
    def testBazelPluginReadParams15(self):
        ...
        v, r = self.bazel_task._read_params()
        self.assertTrue(v)
        self.assertEqual( r, (self.existent_path1, "dummy_value1", "dummy_value2", "dummy_value3", "dummy_value4", True, True, ["dummy_value7"], ["dummy_value8"], "dummy_value9", None, False) )
    ...
    def testBazelPluginReadParams17(self):
        ...
        v, r = self.bazel_task._read_params()
        self.assertTrue(v)
        self.assertEqual( r, (self.existent_path1, "dummy_value1", "dummy_value2", "dummy_value3", "dummy_value4", True, True, ["dummy_value7"], ["dummy_value8"], "dummy_value9", "dummy_value10", False) )
    ...
    def testBazelPluginReadParams19(self):
        ...
        v, r = self.bazel_task._read_params()
        self.assertTrue(v)
        self.assertEqual( r, (self.existent_path1, "dummy_value1", "dummy_value2", "dummy_value3", "dummy_value4", True, True, ["dummy_value7"], ["dummy_value8"], "dummy_value9", "dummy_value10", True) )
    ...
    def testBazelPluginRunTask5(self):
        ...
        local_params = {}
        local_params["exec_path"] = self.existent_path1
        local_params["operation"] = "run"
        self.bazel_task.params = local_params

        with mock.patch("bazel_plugin.CustomTask.task_run", return_value=(True, None)) as dummy:
            v, r = self.bazel_task.run_task(print, "exe_name")
            self.assertTrue(v)
            dummy.assert_called_with(print, self.existent_path1, None, [], None, None, False)

    def testBazelPluginRunTask6(self):
    ...
    def testBazelPluginTaskRun1(self):

        with mock.patch("bazel_wrapper.run", return_value=(False, "dummy-error")) as dummy1:
            with mock.patch("output_backup_helper.dump_outputs_autobackup", return_value=None) as dummy2:
                v, r = self.bazel_task.task_run(print, self.existent_path1, None, [], None, None, False)
                self.assertFalse(v)
                self.assertEqual(r, "dummy-error")
                dummy1.assert_called_with(self.existent_path1, None, [])
                dummy2.assert_not_called()

    def testBazelPluginTaskRun2(self):

        with mock.patch("bazel_wrapper.run", return_value=(True, (True, "", ""))) as dummy1:
            with mock.patch("output_backup_helper.dump_outputs_autobackup", return_value=None) as dummy2:
                v, r = self.bazel_task.task_run(print, self.existent_path1, None, [], None, None, False)
                self.assertTrue(v)
                self.assertEqual(r, None)
    ...
    def testBazelPluginTaskRun3(self):

        with mock.patch("bazel_wrapper.run", return_value=(True, (True, "", ""))) as dummy1:
            with mock.patch("output_backup_helper.dump_outputs_autobackup", return_value=None) as dummy2:
                v, r = self.bazel_task.task_run(print, self.existent_path1, "dummy_value1", [], None, None, False)
                self.assertTrue(v)
                self.assertEqual(r, None)
                dummy1.assert_called_with(self.existent_path1, "dummy_value1", [])
                out_list = [("bazel_plugin_stdout", "", "Bazel's stdout"), ("bazel_plugin_stderr", "", "Bazel's stderr")]
    ...
    def testBazelPluginTaskRun4(self):

        with mock.patch("bazel_wrapper.run", return_value=(True, (True, "", ""))) as dummy1:
            with mock.patch("output_backup_helper.dump_outputs_autobackup", return_value=None) as dummy2:
                v, r = self.bazel_task.task_run(print, self.existent_path1, None, ["dummy_value1", "dummy_value2"], None, None, False)
                self.assertTrue(v)
                self.assertEqual(r, None)
                dummy1.assert_called_with(self.existent_path1, None, ["dummy_value1", "dummy_value2"])
                out_list = [("bazel_plugin_stdout", "", "Bazel's stdout"), ("bazel_plugin_stderr", "", "Bazel's stderr")]
                dummy2.assert_called_with(True, print, out_list)

    def testBazelPluginTaskRun5(self):

        with mock.patch("bazel_wrapper.run", return_value=(True, (True, "test-stdout", ""))) as dummy1:
            with mock.patch("output_backup_helper.dump_outputs_autobackup", return_value=None) as dummy2:
                v, r = self.bazel_task.task_run(print, self.existent_path1, None, [], self.dumped_stdout_file, None, False)
                self.assertTrue(v)
                self.assertEqual(r, None)
                self.assertTrue("test-stdout" in getcontents.getcontents(self.dumped_stdout_file))
                dummy1.assert_called_with(self.existent_path1, None, [])
    ...
    def testBazelPluginTaskRun6(self):

        with mock.patch("bazel_wrapper.run", return_value=(True, (True, "", "test-stderr"))) as dummy1:
            with mock.patch("output_backup_helper.dump_outputs_autobackup", return_value=None) as dummy2:
                v, r = self.bazel_task.task_run(print, self.existent_path1, None, [], None, self.dumped_stderr_file, False)
                self.assertTrue(v)
                self.assertEqual(r, "test-stderr")
                self.assertTrue("test-stderr" in getcontents.getcontents(self.dumped_stderr_file))
                dummy1.assert_called_with(self.existent_path1, None, [])
    ...
    def testBazelPluginTaskRun7(self):

        with mock.patch("bazel_wrapper.run", return_value=(True, (True, "", "test-stderr"))) as dummy1:
            with mock.patch("output_backup_helper.dump_outputs_autobackup", return_value=None) as dummy2:
                v, r = self.bazel_task.task_run(print, self.existent_path1, None, [], None, self.dumped_stderr_file, True)
                self.assertTrue(v)
                self.assertEqual(r, "bazel's stderr has been suppressed")
                self.assertTrue("test-stderr" in getcontents.getcontents(self.dumped_stderr_file))
                dummy1.assert_called_with(self.existent_path1, None, [])
